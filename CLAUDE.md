...
3. **每个集成必须导出纯构造器 `New`**——不碰全局、不读文件、不依赖框架。
   ...
   模块的初始化函数同理，收配置而不是读包级变量：
   `func initAll(ctx context.Context, c Config) (io.Closer, error)`。
   但 `registry.Component.Init` 保持只收 `ctx`——那是所有集成共用的契约，
   配置类型各不相同，加参数只能是 `any`，代价是每个集成都要做一次
   「不可能失败」的类型断言，而包级变量并不会因此消失
   （`xgin.CurrentConfig()` 这类运行期读取还得从它来）。登记时用闭包接上。
